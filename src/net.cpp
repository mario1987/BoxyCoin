...
static const char *strTestNetDNSSeed[][2] = {
    {"test.boxy.online", "testnet-seed.boxy.online"},
    {"test.boxycoin.org", "testnet-seed.boxycoin.org"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Boxy is probably already running."), addrBind.ToString().c_str());
        else
